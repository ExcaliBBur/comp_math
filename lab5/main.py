...
def chooseEquation():
    ...
    while (number != 1 and number != 2):
        ...
        number = int(input("Выберите функцию: "))
    interval = input("\nВведите интервал, на котором будут взяты точки через пробел: ").replace(",", ".").split(" ")
    numberOfDots = int(input("\nВведите количество точек на заданном интервале: "))
    
    while (numberOfDots <= 0):
        print("Количество точек не может быть меньше нуля")
        numberOfDots = int(input("Введите количество точек на заданном интервале: "))
...
def chooseInputType():
    type = input("Откуда брать данные о точках? 1 - консоль; 2 - файл; 3 - на основе выбранной функции\n")
...
def getInputDataFromConsole():
    ...
    for i in range(1, n + 1):
        ...
        data.append(line)
    x = float(input("Введите точку, для которой требуется вычислить приближенное значение: "))
    return data, x
...
def newton(dotsX, dotsY, x):
    ...
    if (x < (dotsX[-1] + dotsX[0]) / 2):
        ...
        print("Исходная точка находится в левой половине отрезка")
        ...
        print("Исходная точка находится в правой половине отрезка")
...
def getPlot(lagr, newt, dotsX, dotsY):
    ...
    p1 = plot(lagr, ('x', dotsX[0], dotsX[-1]), show=False, label = str(lagr1) + " lagrange")
    plt.append(p1[0])
    
    if (newt != None):
        p2 = plot(newt, ('x', dotsX[0], dotsX[-1]), show=False, label = str(newt1) + " newton")
        plt.append(p2[0])
...
def main():
    ...
    if (ansNewt != None):
        print("Многочлен Ньютона с конечными разностями: " + 
          "приближенное значение функции в точке %0.2f = %0.3f\n" % (x_0, ansNewt))
        
    if (ansLagr != None and ansNewt != None):
        print("Результаты, полученные методом Лагранжа и методом Ньютона отличаются на %0.3f" % (abs(ansLagr - ansNewt)))
    if (ansLagr != None):
