...
public class MidRectangleMethod implements Method {
    ...
    public Result compute(double left, double right, double n, double epsilon, Expression expression) {
        ...
        while (true) {
            ...
            double ptr = 0;
            for (int i = 1; i <= n; i++) {
                res1 += expression.F().calculate(((left + ptr) + (left + ptr + h)) / 2);
                ptr += h;
            }
            res1 *= h;
            ptr = 0;
            for (int i = 1; i <= 2 * n; i++) {
                res2 += expression.F().calculate(((left + ptr) + (left + ptr + h)) / 2);
                ptr += h_1;
            }
            res2 *= h_1;
            if (rule.isPerformed(res1, res2, k, epsilon)) break;
            else if (counter >= MAX_ITERATIONS) {
                System.out.println("Метод средних прямоугольников не смог отработать за " + MAX_ITERATIONS + " итераций");
                return null;
            }
            n *= 2;
        }
