    ...
    public void printAnswer(Result result) {
        if (numberOfMethod == 1) {
            switch (modificationOfMethod) {
                case (1) -> {
                    if (result != null)
                        System.out.println("Метод левых прямоугольников отработал за " + result.getN()
                                + " разбиений с результатом " + result.getRes());
                }
                case (2) -> {
                    if (result != null)
                        System.out.println("Метод правых прямоугольников отработал за " + result.getN()
                                + " разбиений с результатом " + result.getRes());
                }
                case (3) -> {
                    if (result != null)
                        System.out.println("Метод средних прямоугольников отработал за " + result.getN()
                                + " разбиений с результатом " + result.getRes());
                }
            }
        } else if (numberOfMethod == 2) {
